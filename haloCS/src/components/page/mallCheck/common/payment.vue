<template>
    <div class="payment_container">
                    ...
                    <div class="payment_aliSelect  clearfix">
                        ...
                        <ul>
                            <li v-for="(item,index) in ali">
                                <el-radio v-model="radio" :label="index"><img width="125" height="40" :src="item.img">
                                </el-radio>
                            </li>
                        </ul>
                    </div>
        ...
        <el-dialog :visible.sync='payFinishDialog' custom-class="payFinishDialog" width="400px">
            <div class="payFinishDialog-text">
                <i class="el-icon-circle-check-outline"></i>已完成支付
            </div>
            <div class="payFinishDialog-btn">
                <span @click="backToIndex">返回商城首页</span>
                <span @click="goDetail">查看订单详情</span>
            </div>
        </el-dialog>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            sureBuy() {
                var reform = []
                if (this.type == 1) {
                    reform.push(this.$store.state.order.product)
                }
                else {
                    reform = this.$store.state.orders.product
                }
                let form = {
                    "id": this.orderId,
                    "products": reform,
            ...
            getData() {
                if (this.type == 1) {
                    this.totalPrice = this.$store.state.order.product.total;
                    this.orderId = this.$store.state.order.id;

                }
                else if (this.type == 2) {
                    this.orderId = this.$store.state.orders.id;
                    var pros = this.$store.state.orders.product;
