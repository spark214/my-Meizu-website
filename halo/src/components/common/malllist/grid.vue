    ...
    methods: {
      ...
      getData(cateId) {
        ...
        if (cateId == -1) {
          ...
          url = this.$rootUrl + "/api/halo/items/search?name=" + name
        }
        else if (cateId == 0) {
          if (brandId !== undefined) {
            url = this.$rootUrl + "/api/halo/items/" + brandId + "/brand"
          }
          else
            url = this.$rootUrl + "/api/halo/items?pageIndex=" + this.pageIndex + "&pageCount=12"
        }
        else {
          if (brandId !== undefined) {
            url = this.$rootUrl + "/api/halo/items/" + cateId + "/cate/" + brandId + "/brand"
          }
          else
            url = this.$rootUrl + "/api/halo/items/" + cateId + "/cate"
        }
        const options = {
          method: 'GET',
