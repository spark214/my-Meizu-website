              ...
              <div v-show="pwdStep==2">
                <el-form :model="pwdForm" :rules="rules" ref="pwdForm" class="mailForm">
                  <el-form-item prop="old">
                    ...
                    <el-input v-model="pwdForm.oldPwd" style="width: 15%" type="password"></el-input>
                  </el-form-item>
                  <el-form-item prop="new">
                    <span class="label">新密码</span>
                    <el-input v-model="pwdForm.newPwd" style="width: 15%" type="password"></el-input>
                  </el-form-item>
...
//遍历匹配
      ...
      updatePwd() {
        ...
        var url = this.$rootUrl + "/api/halo/users/pwd";
        const options = {
          method: 'PATCH',
      ...
      checkPwd() {
        var token = sessionStorage.getItem('accessToken');
        var url = this.$rootUrl + "/api/halo/users/verifyPwd/" + this.pwd;
        const options = {
          method: 'GET',
      ...
      updateEmail() {
        ...
        var url = this.$rootUrl + "/api/halo/users/email";
        const options = {
          method: 'PATCH',
          headers: {'access_token': token, 'Content-Type': 'application/x-www-form-urlencoded'},
          url: url,
      ...
      countDown(item) {
        ...
        if (item == 0)
          var url = this.$rootUrl + "/api/halo/users/requestEmailVerify?email=" + this.mailForm.old;
        else
          var url = this.$rootUrl + "/api/halo/users/requestEmailVerify?email=" + this.mailForm.new;
        var token = sessionStorage.getItem('accessToken');
        const options = {
          method: 'GET',
          url: url,
